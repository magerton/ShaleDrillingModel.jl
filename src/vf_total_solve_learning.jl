...
function solve_vf_infill!(EV::AbstractArray, uin::AbstractArray, wp::well_problem, EVtmp::AbstractArray, logsumubV::AbstractArray, q::AbstractArray, ubVfull::AbstractArray, IminusTEVp::M, Πz::M, β::Real; maxit0::Int=12, maxit1::Int=20, vftol::Real=1e-11) where {M<:AbstractMatrix}
    ...
    for i in infill_state_inds(wp)
        ...
        if horzn == :Finite
            dograd  ||  vfit!(EV0, logsumubV, ubVmax, ubV, Πz)
            # dograd  &&  vfit!(EV0, dEV0, logsumubV, ubV, sumdubV, dubV, Πz)

        elseif horzn == :Infinite
            solve_inf_vfit!(EV0, EVtmp, logsumubV, ubV, Πz, β; maxit=maxit0, vftol=0.1)
            @show converged, iter, bnds = solve_inf_pfit!(EV0, EVtmp, logsumubV, q0, ubV, IminusTEVp, Πz, β; maxit=maxit1, vftol=1e-11)
